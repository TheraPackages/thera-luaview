...
import AtomLuaviewEditor from './atom-luaview-editor';
...
export default class AtomLuaviewServer  extends DebugService {
  ...
  doCmd(socketData){
    ...
    for( ; ; ) {
      ...
      if( cmdName=="running" ) {
        ...
        this.log("[ling] "+ fileName +" : "+lineNumber);
        //this.atomLuaviewEditor.gotoFile(fileName, lineNumber );
        this.sendCmd("stack");
      } else if( cmdName=='log' ) {
  ...
  setupDebugServer() {
    ...
    this.server.on("error",function(){
        self.log('[LuaView][Debugger] Server listening ERROR on ' + HOST +':'+ PORT);
    });
  ...
  addBreakpoint(fileName, lineNumber) {
    if( fileName ) {
      ...
      // }
      {
        ...
        this.sendCmd( "b " + relativeName + ":" + lineNumber );
      }
    }
  ...
  // }
  // atom.workspace.open( "./menus/atom-luaview.json" );
